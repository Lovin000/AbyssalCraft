package com.shinoow.abyssalcraft.core.network;
...
import com.shinoow.abyssalcraft.core.network.packet.AbstractPacket;
...
public class PacketPipeline extends MessageToMessageCodec<FMLProxyPacket, AbstractPacket> {
	...
	public boolean registerPacket(Class<? extends AbstractPacket> clazz) {
		if (packets.size() > 256) {
			CoreLogger.severe("Packet larger than 256!");
			return false;
		}
		if (packets.contains(clazz)) {
			CoreLogger.severe("Registry already contains the package: " + clazz.getCanonicalName() + "!");
			return false;
		}
		if (isPostInitialised) {
			CoreLogger.severe("Postinitializing, REGISTER PACKET EARLIER DAMNIT!");
			return false;
		}
		packets.add(clazz);
		return true;
	...
	public void initialise() {
		channels = NetworkRegistry.INSTANCE.newChannel("AbyssalCraft", this);
	}
